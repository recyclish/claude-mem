...
4. **Worker Service** - Express.js server with the SearchOrchestrator search pipeline
5. **SQLite Database** - Persistent storage with structured filter queries
6. **Chroma Vector DB** - Semantic search for query text
...
Worker service runs the search through the SearchOrchestrator: query text goes
to ChromaDB for semantic ranking, results are filtered by recency/date range,
then hydrated from SQLite with any structured filters applied:

```sql
SELECT * FROM observations
WHERE id IN (/* Chroma-ranked ids */)
AND type = 'bugfix'
LIMIT 10
```
...
**Database Access:**
- Uses `SessionSearch` service for filter-only structured queries
- Uses `SessionStore` for hydration by id
- ChromaDB vector search for semantic similarity on query text

**Filter-Only Search (no query text):**
```typescript
// search tool → HTTP GET → direct SQLite query
SELECT * FROM observations
WHERE type = ?
...
as an embedding input, not executed.

**Testing:** injection attack tests cover special characters, SQL keywords, quote escaping, and boolean operators.
...
**Filter-Only Search:** Sub-10ms direct SQLite queries; semantic search adds a ChromaDB round-trip
...
- [Database Schema](/architecture/database) - Tables and indexes
