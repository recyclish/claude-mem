...
4. **Database Layer** - Store sessions and observations (SQLite + ChromaDB)
...
### Search Pipeline
```
User Query → MCP Tools Invoked → HTTP API → SearchOrchestrator → ChromaDB / SQLite → Search Results → Claude
```
...
4. **SearchOrchestrator**: Worker service runs ChromaDB vector search (query text) or direct SQLite filtering (filter-only)
...
│    Creates session in database, saves raw user prompt           │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
│   │       ├── SessionSearch.ts # Filter-only structured queries
...
- SessionSearch for filter-only structured queries
- ChromaDB mirror for semantic (vector) search
- Location: `~/.claude-mem/claude-mem.db`

See [Database Architecture](/architecture/database) for the schema.

### 4. mem-search Skill (v5.4.0+)
Skill-based search with progressive disclosure providing 10 search operations:
- Search observations, sessions, prompts (semantic vector search)
