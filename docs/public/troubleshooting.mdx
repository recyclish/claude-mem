...
5. If Chroma continues to fail, semantic (query-text) search is unavailable, but filter-only searches (date ranges, types, projects, files) continue to work via direct SQLite queries
...
**Symptoms**: Query-text search returns no results or reports vector search unavailable.

Semantic (query-text) search runs on ChromaDB; there is no keyword-index fallback.
Legacy FTS5 tables from older versions are removed automatically and are not used.
...
2. Verify Chroma is available (requires uv/Python — see the Chroma section above):
   ```bash
   curl "http://localhost:37777/api/search?query=test"
   # "Vector search failed" means Chroma isn't running
   ```

3. Filter-only searches work without Chroma:
   ```bash
   curl "http://localhost:37777/api/search?dateStart=2026-01-01&project=my-project"
   ```
...
4. Restart the worker so ChromaDB re-syncs with the trimmed database:
   ```bash
   npm run worker:restart
   ```
...
**Cause**: ChromaDB is unavailable (uv/Python missing or Chroma failed to start).

**Solution**: Install uv, restart the worker, or use filter-only searches meanwhile.
