...
The `query` parameter is a **semantic search** — plain natural language, matched
by meaning via ChromaDB vector embeddings rather than exact keywords:

```
query="how does authentication work"     # Finds JWT/OAuth/session observations
query="database migration problems"      # Finds related fixes and decisions
query="why we chose SQLite"              # Finds the relevant decision records
```

Describe what you're looking for the way you'd ask a teammate; there is no
boolean or column-prefix syntax. To constrain results structurally, combine the
query with filters (`type`, `project`, `dateStart`/`dateEnd`, `concepts`,
`files`) instead of query operators.
...
1. Be more specific in queries (sharper semantic matches)
...
**Database:** SQLite structured storage and filtering on `~/.claude-mem/claude-mem.db`

**Vector Search:** Chroma embeddings for semantic search on query text
